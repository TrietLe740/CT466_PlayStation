...
exports.delete = async (req, res, next) => {
  try {
    ...
    return res.send({ message: "User was deleted succesfully " });
  } catch (error) {
    return next(
      new ApiError(500, `Could not delete auth with id=${req.params.id}`)
    );
  }
};
...
exports.update = async (req, res, next) => {
  ...
  try {
    ...
    return res.send({ message: "User was updated succesfully" });
  } catch (error) {
    console.log(error);
    return next(
      new ApiError(500, `Error updating auth with id=${req.params.id}`)
    );
  }
};
